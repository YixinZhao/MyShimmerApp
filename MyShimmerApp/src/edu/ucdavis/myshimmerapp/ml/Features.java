...
public class Features {
	...
	private final static IirFilterCoefficients lowPass_coef = getFilterCoef(
			FilterPassType.lowpass, 0.1, 0);
	private final static IirFilterCoefficients bandPass_coef = getFilterCoef(
			FilterPassType.lowpass, 0.01, 0.468);
	private final static IirFilterCoefficients bandEnergy_coef = getFilterCoef(
			FilterPassType.lowpass, 0.03, 0.35);
	private final static IirFilterCoefficients lowEnergy_coef = getFilterCoef(
			FilterPassType.lowpass, 0.07, 0);
	private final static IirFilterCoefficients modvigEnergy_coef = getFilterCoef(
			FilterPassType.lowpass, 0.071, 0.468);
	...
	public Features(MyShimmerDataList input) {
		if (input != null && !input.isEmpty()) {

			int dataSize = input.size();
			Log.d(TAG, "construct features:" + dataSize);
			...
			for (int i = 0; i < 6; i++) {
				List<Double> serie = input.getSerie(i);
				for (int j = 0; j < input.size(); j++) {
					datas[i][j] = serie.get(j).doubleValue();
				}
			}
	...
	public FeatureClass getFeatures() {
		return feature;
	}

	private static IirFilterCoefficients getFilterCoef(
			FilterPassType filter_type, double fcf1, double fcf2) {
		IirFilterCoefficients coef = IirFilterDesignFisher.design(filter_type,
				FilterCharacteristicsType.butterworth, 4, 0, fcf1, fcf2);
		for (double d : coef.a) {
			Log.d(TAG, "a:" + d);
		}
		for (double d : coef.b) {
	...
	private double[] filtfilt(IirFilterCoefficients coef, double[] input) {
		...
		if (input != null && input.length != 0) {
			...
			{
				Log.d(TAG, "test fft before filter");
			...
			{
				Log.d(TAG, "test fft after filter");
				Complex[] data_fft = doFFT(ret);
				double[] tmp = new double[data_fft.length];
				for (int i = 0; i < data_fft.length; i++) {
					tmp[i] = 20 * FastMath.log10(data_fft[i].abs());
				}
				String LogFileName = "fft_after" + System.currentTimeMillis()
						+ ".csv";
				String logFilePath = Environment.getExternalStorageDirectory()
						+ "/ShimmerTest/";
				File LogFile = new File(logFilePath, LogFileName);
				Log.d(TAG, "fft_after:" + LogFileName);
				try {
					BufferedWriter buf = new BufferedWriter(new FileWriter(
							LogFile, true));
					for (int i = 0; i < tmp.length; i++) {
	...
	private void calcFeatures() {
		for (int i = 0; i < 6; i++) {
			...
			for (Complex c : data_band_fft[i]) {
				Log.d(TAG, "fft:" + i + ":" + +c.abs());
			}
